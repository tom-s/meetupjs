...
class OpticalFlow extends Component {
  ...
  componentDidMount = () => {
    window.navigator.mediaDevices.getUserMedia({ audio: false, video: true}).then(this.videoReady).catch(this.videoError)
  ...
  render = () => 
    <div className='Oflow'>
      <div className='Oflow_wrapper'>
        <video style={{display: 'none'}} ref={el => this.video = el} className='Oflow_video' width="640px" height="480px" autoPlay loop muted></video>
        <canvas ref={el => this.canvas = el} className='Oflow_canvas' width='640px' height='480px'></canvas>
      </div>
    </div>
}
